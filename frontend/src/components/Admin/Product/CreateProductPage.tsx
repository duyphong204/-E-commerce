...
import { CreateProductPayload, ProductImage } from "../../../types";
...
const CreateProductPage: React.FC = () => {
    ...
    try {
      const uploads = await Promise.all(
        files.map(async (file) => {
          ...
          const { data } = await axios.post<{ url?: string; imageUrl?: string }>(
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      NotificationService.error(errObj.message || "Tạo sản phẩm thất bại");
