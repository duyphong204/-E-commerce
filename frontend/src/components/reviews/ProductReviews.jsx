            ...
            <div className="flex items-center gap-1 mt-2">
              {[1, 2, 3, 4, 5].map((star) => {
                ...
                return (
                  <Star 
                    key={star} 
                    className={`w-5 h-5 ${isFull ? "fill-yellow-400 stroke-yellow-400" : isHalf ? "fill-yellow-200 stroke-yellow-400" : "fill-none text-gray-200"}`} 
                  />
                );
              })}
        ...
        <div className="lg:w-3/5 space-y-8">
          ...
          <div className="space-y-6 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin">
            ...
            {reviews.length === 0 && !loading && (
              <p className="text-sm text-gray-400 font-medium italic">Sản phẩm chưa có đánh giá nào. Hãy là người đầu tiên chia sẻ cảm nhận!</p>
            )}
                ...
                <div key={r._id} className="group border-b border-gray-50 pb-5 last:border-0 last:pb-0 transition-all duration-300">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      ...
                      {/* Avatar initials */}
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center font-extrabold text-sm ${getAvatarColor(userName)}`}>
                        {initials}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star 
                            key={star} 
                            className={`w-3.5 h-3.5 ${star <= r.rating ? "fill-yellow-400 stroke-yellow-400" : "fill-none text-gray-200"}`} 
                          />
                        ))}
                      </div>
