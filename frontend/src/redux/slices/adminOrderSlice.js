...
export const fetchAllOrders = createAsyncThunk(
  ...
  async ({ page = 1, limit = 10 }, { rejectWithValue }) => {
    try {
      const { data } = await axios.get(`${API_URL}/api/admin/orders?page=${page}&limit=${limit}`);
      return data;
    } catch (err) {
...
export const searchOrder = createAsyncThunk(
  ...
  async ({ term, page = 1, limit = 10 }, { rejectWithValue }) => {
    try {
      const { data } = await axios.get(`${API_URL}/api/admin/orders/search?term=${term}&page=${page}&limit=${limit}`);
      return data;
    } catch (err) {
...
export const updateOrderStatus = createAsyncThunk(
  ...
  async ({ id, status }, { rejectWithValue }) => {
    try {
      const { data } = await axios.put(`${API_URL}/api/admin/orders/${id}`, { status });
      return data;
    } catch (err) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAllOrders.fulfilled, (state, action) => {
        ...
        state.processingCount = action.payload.results.filter(
