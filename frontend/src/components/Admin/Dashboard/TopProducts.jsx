    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col h-full">
                ...
                <ul className="space-y-4 flex-1">
                    {topSelling.length > 0 ? topSelling.map((product) => (
                        <li key={product._id} className="flex items-center justify-between group hover:bg-gray-50/50 p-2 -mx-2 rounded-xl transition-colors">
                            <div className="flex items-center gap-3 overflow-hidden">
                                ...
                                <div className="truncate">
                                    <p className="text-sm font-bold text-gray-900 group-hover:text-amber-600 transition-colors truncate" title={product.name}>
                                        {product.name}
                                    </p>
                                    <p className="text-xs text-gray-500 font-medium">Đã bán: <span className="text-gray-700">{product.totalSold}</span></p>
                                </div>
                            </div>
                    ...
                    {lowStock.length > 0 ? lowStock.map((product) => (
                        <li key={product._id} className="flex items-center justify-between group hover:bg-gray-50/50 p-2 -mx-2 rounded-xl transition-colors">
                            <div className="flex items-center gap-3 overflow-hidden">
                                ...
                                />
                                <div className="truncate">
                                    <p className="text-sm font-bold text-gray-900 group-hover:text-red-600 transition-colors truncate" title={product.name}>
                                        {product.name}
                                    </p>
                                </div>
                            </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col h-full">
                ...
                <ul className="space-y-4 flex-1">
                    {topWishlist.length > 0 ? topWishlist.map((product) => (
                        <li key={product._id} className="flex items-center justify-between group hover:bg-gray-50/50 p-2 -mx-2 rounded-xl transition-colors">
                            <div className="flex items-center gap-3 overflow-hidden">
                                <img
                                    src={product.image || "/no-image.png"}
                                ...
                                />
                                <div className="truncate">
                                    <p className="text-sm font-bold text-gray-900 group-hover:text-pink-600 transition-colors truncate" title={product.name}>
                                        {product.name}
                                    </p>
                                </div>
                            </div>
