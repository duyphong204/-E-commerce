...
import { useAdminProducts, useProductMutations } from '../api/useProducts';
...
export function AdminProductManagementContainer() {
  const [page, setPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState('');

  const { data, isLoading, isFetching, error } = useAdminProducts({ page, term: searchTerm, limit: 10 });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      NotificationService.error(getErrorMessage(err, 'Xóa sản phẩm thất bại'));
    }
  };
  ...
  const { activeCount = 0, lowStockCount = 0 } = data?.statistics || {};
...
export default AdminProductManagementContainer;
