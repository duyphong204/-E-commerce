...
const FilterSlideBar: React.FC = () => {
  ...
  const materials = ["Cotton", "Len", "Denim", "Polyester", "Lụa", "Linen"];
  ...
  const updateURLParams = (newFilters: FilterState): void => {
    ...
    (Object.keys(newFilters) as (keyof FilterState)[]).forEach((key) => {
      ...
      } else if (typeof val === "number" && val !== 0 && key !== "minPrice") {
  ...
  const handleFilterChange = (e: { target: { name: string; value: string; checked?: boolean; type?: string } } | ChangeEvent<HTMLInputElement>): void => {
    const target = e.target;
    const name = target.name as keyof FilterState;
    ...
    if (type === "checkbox") {
      const currentList = (newFilters[name] as string[]) || [];
      if (checked) {
        (newFilters[name] as string[]) = [...currentList, value];
      } else {
        (newFilters[name] as string[]) = currentList.filter((item) => item !== value);
      }
    } else {
      (newFilters[name] as unknown) = value;
    }
          ...
          {sizes.map((size) => {
            ...
            return (
              <button
                ...
                key={size}
                onClick={() => {
                  const newSize = isSelected ? filters.size.filter((s) => s !== size) : [...filters.size, size];
        ...
        <div className="space-y-2.5">
          {materials.map((material) => {
            ...
            return (
              <label
                key={material}
                className="flex items-center gap-2.5 text-xs sm:text-sm font-semibold text-gray-500 cursor-pointer group"
              >
                <div
                  className={`w-4.5 h-4.5 rounded-md border flex items-center justify-center transition-all duration-200 ${
                    isSelected ? "bg-emerald-500 border-transparent text-white" : "bg-white border-gray-200 group-hover:border-gray-400"
                  }`}
                >
                  {isSelected && (
                ...
                <span
                  className={`transition-colors duration-200 ${
                    isSelected ? "text-gray-900 font-bold" : "group-hover:text-gray-900"
                  }`}
                >
                  {material}
                </span>
